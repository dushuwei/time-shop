...
function toggleSound() {
    ...
    if (document.getElementById('sound-status')) {
...
function checkAchievements() {
    const achievements = [
        { id: 'first_sale', name: '第一笔交易', description: '完成第一笔交易', icon: '💰', condition: () => gameState.unlockedStories.length >= 1 },
        { id: 'story_collector', name: '故事收集者', description: '解锁10个故事', icon: '📖', condition: () => gameState.unlockedStories.length >= 10 },
        { id: 'wealthy_merchant', name: '富商', description: '拥有1000金币', icon: '💎', condition: () => gameState.money >= 1000 },
        { id: 'famous_shop', name: '名店', description: '声望达到100', icon: '⭐', condition: () => gameState.reputation >= 100 },
        { id: 'perfect_service', name: '完美服务', description: '连续10次交易成功', icon: '🏆', condition: () => gameState.consecutiveSuccess >= 10 }
    ];
...
function showAchievement(achievement) {
    ...
    popup.innerHTML = `<h3>${achievement.icon} 成就解锁！</h3><p><strong>${achievement.name}</strong></p><p>${achievement.description}</p>`;
...
function showAchievements() {
    ...
    achievements.forEach(ach => {
        ...
        html += `<div class="story-item ${unlocked ? '' : 'locked'}"><h4>${ach.icon} ${ach.name}</h4><p>${ach.description}</p><p style="color: ${unlocked ? '#4ade80' : '#666'};">${unlocked ? '✅ 已解锁' : '🔒 未解锁'}</p></div>`;
    });
...
function showStoryBook() {
    ...
    CUSTOMERS.forEach(customer => {
        ...
        html += `<div class="story-item ${unlocked ? '' : 'locked'}"><h4>${customer.avatar || '❓'} ${unlocked ? customer.name : '???'}</h4><p style="font-style: italic; color: #daa520;">${unlocked ? customer.storyTitle : '未解锁'}</p><p>${unlocked ? customer.era : '需要满足特定条件'}</p></div>`;
    });
    html += '</div><button onclick="this.closest(\'.merchant-overlay\').remove()">关闭</button></div></div>';
    document.body.insertAdjacentHTML('beforeend', html);
}
...
function showUpgrades() {
    ...
    upgrades.forEach(upgrade => {
        ...
        html += `<div class="upgrade-card ${purchased ? 'purchased' : ''}"><div><div style="font-size: 1.5rem;">${upgrade.icon} ${upgrade.name}</div><div style="color: #daa520; font-size: 0.9rem;">${upgrade.description}</div><div style="color: #ffd700; margin-top: 0.5rem;">💰 ${upgrade.cost} 金币</div></div><button onclick="buyUpgrade('${upgrade.id}', ${upgrade.cost})" ${purchased || !canAfford ? 'disabled' : ''}>${purchased ? '已购买' : (canAfford ? '购买' : '金币不足')}</button></div>`;
    });
    html += '</div><button onclick="this.closest(\'.merchant-overlay\').remove()">关闭</button></div></div>';
    document.body.insertAdjacentHTML('beforeend', html);
}
...
function openMerchant() {
    const merchantItems = [
        { itemId: 'ancient_book' }, { itemId: 'sundial' }, { itemId: 'telescope' },
        { itemId: 'typewriter' }, { itemId: 'music_box' }, { itemId: 'painting' }
    ];
    ...
    merchantItems.forEach(item => {
        ...
        if (itemData) {
            html += `<div class="merchant-item"><div style="font-size: 3rem;">${itemData.icon}</div><div style="font-size: 1.2rem; color: #ffd700; margin: 0.5rem 0;">${itemData.name}</div><div style="color: #daa520; font-size: 0.9rem;">${itemData.description}</div><div style="color: #ffd700; margin-top: 0.5rem;">💰 ${itemData.price} 金币</div><button onclick="buyFromMerchant('${item.itemId}', ${itemData.price})">购买</button></div>`;
        }
    });
...
function initGame() {
    ...
    if (document.getElementById('total-stories')) {
        document.getElementById('total-stories').textContent = CUSTOMERS.length;
    }
}
...
function startNewGame() {
    playSound('button');
    gameState = {
        money: 100, day: 1, reputation: 0,
        inventory: {
            cassette_tape: 2, hologram_toy: 1, pocket_watch: 1,
            polaroid_camera: 1, game_console: 1, love_letter: 1,
            vinyl_record: 1, snow_globe: 1
        },
        currentCustomer: null, dialogueIndex: 0, unlockedStories: [],
        achievements: [], shopUpgrades: [], consecutiveSuccess: 0,
        merchantVisited: false, soundEnabled: true
    };
    enterShop();
}
    ...
    for (const [itemId, quantity] of Object.entries(gameState.inventory)) {
        if (quantity > 0) {
            ...
            if (item) {
                ...
                itemDiv.innerHTML = `<div class="item-icon">${item.icon}</div><div class="item-info"><div class="item-name">${item.name}</div><div class="item-quantity">数量: ${quantity}</div></div>`;
...
function showDialogue() {
    ...
    if (gameState.dialogueIndex < gameState.currentCustomer.dialogue.length) {
        document.getElementById('dialogue-text').textContent = gameState.currentCustomer.dialogue[gameState.dialogueIndex];
        playSound('dialogue');
    } else {
        showCustomerRequest();
    }
...
function nextDialogue() {
    playSound('button');
    if (!gameState.currentCustomer) { spawnCustomer(); return; }
...
function offerItem(itemId) {
    playSound('button');
    if (!gameState.currentCustomer) { showMessage('现在没有顾客...', 'error'); return; }
    if (gameState.dialogueIndex < gameState.currentCustomer.dialogue.length) {
...
function nextDay() {
    ...
    if ([3, 7, 12, 18, 25].includes(gameState.day) && !gameState.merchantVisited) {
        ...
        setTimeout(() => { openMerchant(); gameState.merchantVisited = true; }, 1500);
    } else {
